...
class FileLog implements LoggerInterface {
   ...
   */
  protected ?LogFileManagerInterface $fileManager = NULL;
   ...
   *   The dependency injection container used for deferred services.
   */
  public function __construct(ConfigFactoryInterface $configFactory, TimeInterface $time, LogMessageParserInterface $parser, ContainerInterface $container) {
  ...
  public function log($level, $message, array $context = []): void {
    ...
    try {
      ...
      if (($context['channel'] ?? '') !== 'filelog') {
        fwrite($this->stderr(), 'FileLog exception: ' . $error->getMessage() . "\n");
      }
  ...
  /**
   * Gets the filelog configuration without loading overrides at construction.
   */
  protected function config(): Config {
    if (!isset($this->config)) {
      $this->config = $this->configFactory->get('filelog.settings');
    }
    return $this->config;
  }

  /**
   * Gets the lazily loaded state service.
   */
  ...
  /**
   * Gets the lazily loaded log-file manager.
   */
  protected function fileManager(): LogFileManagerInterface {
    if (!isset($this->fileManager)) {
      $this->fileManager = $this->getContainer()->get('filelog.file_manager');
    }
